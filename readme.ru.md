...
Это веб-приложение обрабатывает большие текстовые блоки: оно делит их на управляемые фрагменты и передаёт каждый фрагмент в модель Google Gemini (сейчас — Gemini Flash 2.0). Интерфейс позволяет вставлять текст вручную или загружать файлы с подсказками и исходным содержимым, управлять лимитами запросов и получать итоговый ответ в одном объединённом файле.
...
- Настройка ограничения скорости запросов (requests per minute)
- Сохранение ответов каждого фрагмента в отдельные файлы
- Загрузка подсказок и исходного текста из локальных файлов
- Получение одного объединённого файла с итоговым ответом
- Веб-интерфейс с настройками размера фрагмента и параллельности
...
## Предпочитаемая установка (Docker)

Самый быстрый способ начать — запустить проект в Docker: зависимости остаются изолированными, и это рекомендуемая конфигурация.
...
3. В веб-интерфейсе можно настроить или загрузить:
   - Подсказку для модели (вставьте текст или выберите файл)
   - Исходный текст (вставьте текст или выберите файл)
...
- **Разделитель**: шаблон для разбиения текста на фрагменты (например, `\n\n` для абзацев)
- **Параллельные запросы**: количество фрагментов, обрабатываемых одновременно
- **Максимум запросов в минуту**: ограничение скорости запросов к API (при необходимости)
- **Размер фрагмента**: максимальная длина одного фрагмента в символах

## Результат

- Обработанные ответы отображаются прямо в интерфейсе
- Объединённый файл с результатом доступен по ссылке для скачивания
...
- Добавлены комментарии, если без них сложно понять изменения
- Изменения протестированы перед отправкой
